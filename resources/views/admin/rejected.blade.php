...
<head>
    ...
    <title>Job Admin Panel - Rejected</title>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 font-sans">
    <div class="flex min-h-screen">
        <div class="w-64 bg-gradient-to-b from-orange-500 to-orange-600 p-6 shadow-xl slide-in">
            ...
            <nav class="space-y-3">
                ...
                <a href="#" onclick="setActiveTab('approved', event)" id="nav-approved" class="nav-item block w-full px-4 py-3 bg-orange-300 bg-opacity-50 text-white rounded-lg text-center hover:bg-opacity-70 transition-all duration-300 transform hover:scale-105">
                     <span class="flex items-center justify-center space-x-2">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                        <span>Approved</span>
                    </span>
                </a>
                <a href="#" onclick="setActiveTab('rejected', event)" id="nav-rejected" class="nav-item active block w-full px-4 py-3 bg-white text-orange-500 rounded-lg text-center font-medium hover:bg-gray-50 transition-all duration-300 transform hover:scale-105 shadow-lg">
                    <span class="flex items-center justify-center space-x-2">
            ...
            <div class="mt-8 bg-white bg-opacity-20 backdrop-blur-sm rounded-lg p-4 fade-in">
                ...
                <div class="space-y-2 text-white text-xs">
                    ...
                    <div class="flex justify-between">
                        <span>Rejected:</span>
                        <span id="rejectedCount" class="font-bold text-red-200">8</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 p-8">
            ...
            <div class="bg-white rounded-lg shadow-xl overflow-hidden fade-in">
                <div class="flex justify-between items-center p-6 border-b border-gray-200 bg-gradient-to-r from-orange-50 to-orange-100">
                    <h2 class="text-2xl font-semibold text-gray-800">Rejected Table</h2>
                    ...
                    <table class="w-full" id="jobTable">
                        <thead class="bg-gradient-to-r from-orange-200 to-orange-300">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-sm font-medium text-gray-700 border-r border-orange-400">Rejected</th>
    ...
    <script>
        // Sample data for Rejected view
        let jobData = [
            {id: 1, jobName: "Job #1", company: "Company A", category: "Admin & Operations", email: "Company1@email.com", status: "rejected"},
            {id: 2, jobName: "Job #2", company: "Company B", category: "Business Dev & Sales", email: "Company2@email.com", status: "rejected"},
            {id: 3, jobName: "Job #3", company: "Company C", category: "CS & Hospitality", email: "Company3@email.com", status: "rejected"},
            {id: 4, jobName: "Job #4", company: "Company D", category: "Data & Product", email: "Company4@email.com", status: "rejected"},
            {id: 5, jobName: "Job #5", company: "Company E", category: "Design & Creative", email: "Company5@email.com", status: "rejected"},
            {id: 6, jobName: "Job #6", company: "Company F", category: "Education & Training", email: "Company1@email.com", status: "rejected"},
            {id: 7, jobName: "Job #7", company: "Company G", category: "Finance & Accounting", email: "Company6@email.com", status: "rejected"},
            {id: 8, jobName: "Job #8", company: "Company H", category: "Food & Beverage", email: "Company7@email.com", status: "rejected"}
        ];

        let filteredData = [...jobData]; // In this view, all data is pre-filtered to 'rejected'
        let sortDirection = {};

        document.addEventListener('DOMContentLoaded', function() {
            renderTable();
            updateStats(); // To update sidebar stats if needed
            setActiveTab('rejected'); // Set this page's tab as active
        });
            ...
            filteredData.forEach((job, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-6 py-4 text-sm text-gray-900 border-r border-gray-200 text-center">
                        <span class="bg-red-500 text-white px-3 py-1 rounded-full text-xs font-medium">Rejected</span>
                    </td>
        ...
        function updateStats() {
            document.getElementById('totalJobs').textContent = jobData.length;
            document.getElementById('approvedCount').textContent = 0;
            document.getElementById('rejectedCount').textContent = jobData.length;
        }
        ...
        function showJobDetails(jobId) {
            ...
            if (job) {
                ...
                setTimeout(() => modalContentContainer.classList.add('scale-100'), 10);
        ...
        function setActiveTab(tabName, event) {
            ...
            // if (tabName === 'approved') window.location.href = 'approved_jobs_ui.blade.php';
        }
    </script>
</body>
</html>