...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <h1 class="text-2xl font-semibold mb-6 text-gray-700 font-figtree ">Daftar Ajuan Lowongan (Processed)</h1>
                        ...
                        @forelse ($lowonganList as $lowongan)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 font-medium">
                                    {{ $lowongan->job_name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $lowongan->company_name }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                    {{ $lowongan->category_job ?? 'N/A' }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center space-x-2">
                                    ...
                                    @if ($lowongan->status != 'accept')
                                        <form action="{{ route('admin.processed.update_status', $lowongan->id) }}"
                                            method="POST" class="inline-block">
                                            @csrf
                                            @method('PATCH')
                                            <input type="hidden" name="status" value="accept">
                                            <button type="submit"
                                                class="px-3 py-1.5 text-xs font-medium text-white bg-green-500 rounded-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
                                                Approve
                                            </button>
                                        </form>
                                    @endif
                                    ...
                                    @if ($lowongan->status != 'decline')
                                        <form action="{{ route('admin.processed.update_status', $lowongan->id) }}"
                                            ...
                                            <button type="submit"
                                                class="px-3 py-1.5 text-xs font-medium text-white bg-red-500 rounded-md hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
                                                Reject
                                            </button>
                                        </form>
                                    @endif
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                    @if (Route::has('detail_job'))
                                        <a href="{{ route('detail_job', $lowongan->id) }}"
                                            class="px-3 py-1.5 text-xs font-medium text-white bg-orange-500 rounded-md hover:bg-orange-600">
                                            Detail
                                        </a>
                                    @endif
                                </td>
