        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 max-w-5xl mx-auto p-8 sm:p-10 space-y-8">
            ...
            <div class="text-center mb-8">
                <h2 class="text-4xl font-extrabold text-gray-900 mb-4">
                    {{ $job->company->name ?? 'Nama Perusahaan' }}
            ...
            <div class="flex flex-col md:flex-row gap-8 items-start">
                ...
                <div class="flex-shrink-0 bg-[#E5F3FF] rounded-xl w-36 h-36 md:w-44 md:h-44 flex items-center justify-center overflow-hidden shadow-lg">
                    @if ($job->company && !empty($job->company->logo_path) && file_exists(public_path('storage/' . $job->company->logo_path)))
                    <img src="{{ asset('storage/' . $job->company->logo_path) }}"
                        alt="Logo {{ $job->company->name ?? 'Perusahaan' }}"
                        class="object-cover w-full h-full rounded-xl">
                    @else
                    <p class="text-xs text-red-500">Logo Gagal Dimuat.</p>
                    <p class="text-xs text-red-500">Path: {{ $job->company->logo_path ?? 'Path tidak tersedia' }}</p>
                    <p class="text-xs text-red-500">File Exists?: {{ ($job->company && !empty($job->company->logo_path) && file_exists(public_path('storage/' . $job->company->logo_path))) ? 'Ya' : 'Tidak' }}</p>
                    <p class="text-xs text-red-500">Cek $job->company: {{ $job->company ? 'Ada' : 'Tidak Ada' }}</p>
                    <p class="text-xs text-red-500">Cek $job->company->logo_path: {{ !empty($job->company->logo_path) ? 'Ada Isi' : 'Kosong' }}</p>
                ...
                <div class="flex-1 space-y-2">
                    ...
                    <p class="text-sm text-gray-600 mt-2">
                        {{ $job->company->description ?? 'Deskripsi perusahaan...' }}
                    </p>
                </div>
            </div>
            ...
            <div>
                ...
                <div
                    ...
                    $job->description?? '<p>Detail deskripsi pekerjaan...</p>',
                    ) !!}
                </div>
            </div>
            ...
            <div class="mt-6">
                ...
                <div
                    class="prose max-w-none text-gray-700 [&>ul]:list-disc [&>ol]:list-decimal [&>ul]:pl-5 [&>ol]:pl-5 [&>li]:mb-2">
                    {!! App\Helpers\HtmlHelper::cleanJobHtml(
                    $job->requirements ?? '<p>Detail persyaratan pekerjaan...</p>',
                    ) !!}
                </div>
            </div>
            ...
            <div class="bg-yellow-50 rounded-2xl p-6 mt-8">
                <table class="w-full text-sm text-gray-800">
                    @php
                    $info = [
                    'Tingkat Pengalaman' => !empty($job->experience_years) ? $job->experience_years . ' Tahun' : 'Kurang dari 1 Tahun',
                    'Formulir Online' =>
                    $job->apply_link ?? 'https://forms.gle/abc1234EFGH5678',
                    ];
                    @endphp
            ...
            <div class="flex items-start space-x-4 mb-6">
                ...
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">Lamar
                        {{ $jobData['step1']['job_name'] ?? 'Pekerjaan Ini' }}
                    </h3>
    ...
    function shareJob() {
        if (navigator.share) {
            navigator.share({
                ...
                step1 ']['
                job_name '] ?? '
                Lowongan Kerja ' }} - {{ $jobData['
                step4 ']['
                company_name '] ?? '
                ' }}',
                text: 'Lihat lowongan kerja ini: {{ $jobData['
                step1 ']['
                job_name '] ?? '
                ' }}',
